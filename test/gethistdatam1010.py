...
def inserthistdata(mylogger, board, df, value_code, x, period):
...
def gethistdatam(mylogger, board, value_code, startrows, endrows, startday, endday):
    b = 0
    global totalknums
    global stocknums
    global mrlock
    ...
    for x in range(startrows, endrows):
        ...
        if startday == 'all':
            ...
            # print(df)
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, 'D')
                b += datanum
            df = ts.get_hist_data(value_code[x][0], ktype='W')
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, 'W')
                b += datanum
            df = ts.get_hist_data(value_code[x][0], ktype='M')
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, 'M')
                b += datanum
            df = ts.get_hist_data(value_code[x][0], ktype='5')
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, '5')
                b += datanum
            df = ts.get_hist_data(value_code[x][0], ktype='15')
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, '15')
                b += datanum
            df = ts.get_hist_data(value_code[x][0], ktype='30')
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, '30')
                b += datanum
            df = ts.get_hist_data(value_code[x][0], ktype='60')
            if df is not None:
                datanum = inserthistdata(mylogger, board, df, value_code, x, '60')
                b += datanum
        else:
        ...
        mylogger.info("stock_hist_data_" + board + " 股票%s%s 共插入%d条数据。该板目前完成股票数量为%d，插入总条数为%d，完成进度为%.2f%%" % (value_code[x][0], value_code[x][1], b, stocknums, totalknums, progress_percent))
        print("stock_hist_data_" + board + " 股票%s%s 共插入%d条数据。该板目前完成股票数量为%d，插入总条数为%d，完成进度为%.2f%%" % (value_code[x][0], value_code[x][1], b, stocknums, totalknums, progress_percent))
        mrlock.release()
        b = 0
...
def starthreads(mylogger, board, stockcode, startday, endday):
    ...
    if stockcode == 'all':
        ...
        if threadnums == 0:
            ...
            threads.append(
                threading.Thread(target=gethistdatam, args=(mylogger, board, value_code, startrows, endrows, startday, endday)))
        else:
            startrows = 0
            cid = 1
            while cid <= threadnums:
                ...
                # print(endrows)
                threads.append(threading.Thread(target=gethistdatam, args=(mylogger, board, value_code, startrows, endrows, startday, endday)))
                startrows = cid * step
                # print(startrows)
                cid += 1
        ...
        endrows = 1
        print(value_code)
        gethistdatam(mylogger, board, value_code, startrows, endrows, startday, endday)
